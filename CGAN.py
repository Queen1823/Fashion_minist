# -*- coding: utf-8 -*-
"""
Created on Tue Sep 28 20:27:21 2021
...
train_labels = tf.one_hot(train_labels, depth=10)  # 将标签进行独热编码
...
def make_generator_model():
    ...
    model.add(layers.Dense(7*7*256, use_bias=False, input_shape=(110,)))   # 因为加入了标签信息，input_shape发生改变
...
def make_discriminator_model():
    ...
    model.add(layers.Conv2D(64, (5, 5), strides=(2, 2), padding='same',
                            input_shape=[28, 28, 11]))   # input_shape加了标签信息
    model.add(layers.LeakyReLU())
...
labels = tf.one_hot(labels, depth=10)    # 将标签进行独热编码
...
def train_step(data_batch):
    ...
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
        noise = tf.random_normal([images.get_shape()[0], noise_dim])  # 产生噪音图像
        noise_input = tf.concat([noise, labels], 1)  # 噪音图像数据连接标签数据
        generated_images = generator(noise_input, training = True)  # 生成图像

        labels_input = tf.reshape(labels, [images.get_shape()[0], 1, 1, 10])  # 标签数据reshape
             # 真实图像数据连接上标签数据
